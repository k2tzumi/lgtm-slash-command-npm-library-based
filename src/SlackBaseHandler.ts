...
abstract class SlackBaseHandler<T> {
  ...
  public abstract handle(
    e: DoPost
  ): { performed: boolean; output: TextOutput | null };

  public addListener(type: string, handler: T): void {
    this.listners.set(type, handler);
  }

  protected getListener(type: string): T | undefined {
  ...
  protected validateVerificationToken(token: string | null): void {
    if (token !== this.verificationToken) {
      if (this.verificationToken === "") {
        throw new Error("The configured verification token is empty.");
      } else {
  ...
  protected convertJSONOutput(response: any | null | void): TextOutput {
    if (response) {
